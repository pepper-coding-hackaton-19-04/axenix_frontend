    ...
    <ContainerUI typeContainer='xlarge'>
      <VStack alignItems='start'>
        ...
        <VStack alignItems='start'>
            ...
            {!createdDownloadLink && ( 
                    <button  style={{paddingLeft: '30px', paddingTop: '30px', fontWeight: '600', color: 'orange'}} onClick={createDownloadLink}>Получить ссылку на скачивание</button>
                )}
                {downloadLink && (
                    <p>
            ...
            {data.map((row, index) => (
              <tr key={index} >
                {Object.values(row).map((value, innerIndex) => (
                    //@ts-ignore
                  <td key={innerIndex} style={{ border: '1px solid black' }}>{value}</td>
